...
def preset_model(args, cfg, model, logger, test_type):
    if args.ckpt is not None:
        checkpoint_dir = os.path.join(args.results_dir, cfg.backbone, args.dataset_name, args.log_name, 'snapshots',
                                      args.ckpt)
    ...
    with torch.no_grad():
        ...
        for steps, (image, labels) in enumerate(tqdm(val_dataloader)):
            ...
            for i in range(cfg.max_position_embeddings - 1):
                ...
                predictions, loss_vae = model(image, caption, cap_mask)
...
def main_worker(gpu, args, cfg):
    ...
    model = Seq_transformer.build_model(cfg)
