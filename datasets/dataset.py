...
class SeqDeepFakeDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.mode in ["train"]:
            ...
            label_list = [int(label_list_split[0][1]), int(label_list_split[1][1]), int(label_list_split[2][1]),
                          int(label_list_split[3][1]), int(label_list_split[4][1])]

            caption = np.array(label_list)

            #reverse the prediction order
            zero_index = -1
            # Find the first zero value index
            for i, value in enumerate(caption):
                if value == 0:
                    zero_index = i
                    break
            # If a zero value is found, reverse the part before it
            if zero_index >= 0:
                caption[:zero_index] = caption[:zero_index][::-1]

            # If no zero value found, reverse the whole array
            else:
                caption = caption[::-1]
            ...
            # If a zero value is found, reverse the part before it
            if zero_index >= 0:
                label_list[:zero_index] = label_list[:zero_index][::-1]
