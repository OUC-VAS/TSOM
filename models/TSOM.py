...
from .DPDA_MSCA_block import build_transformer


class TSOM(nn.Module):
    ...
    def forward(self, samples, target, target_mask):
        ...
        hs, loss_vae = self.transformer(src, mask,
                              pos[-1], target, target_mask, h_w, image)
...
def build_model(config):
    ...
    model = TSOM(backbone, transformer, config.hidden_dim, config.vocab_size, config.imgsize)
