...
#SBATCH --cpus-per-task=4 # 需要使用多少CPU，总共有24个CPU。建议使用12个CPU。
#SBATCH -o slurm.output # 把输出结果STDOUT保存在哪一个文件
#SBATCH -e slurm.error # 把报错结果STDERR保存在哪一个文件
